...
model = model_builder() #what am I suppose to feed into model_builder?

tuner = kt.Hyperband(model, #what am I suppose to feed into model_builder?
                     objective = 'val_accuracy',
                     max_epochs = num_topics,
                     factor = 3,
                     directory = 'my_dir',
...
tuner.search(comments, comment_score_normalized, epochs = 10, validation_data = (comments_predict, post_ids_predict), callbacks = [ClearTrainingOutput()])
...
for i in range(iters):
    ...
    for i in range(temp):
        ...
        model.fit(X, post_weights.T, epochs = num_topics, validation_data = (comments_predict, post_ids_predict))
